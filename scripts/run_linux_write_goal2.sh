...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'run_linux_write_goal2.sh must run inside the fixed Linux VM\n' >&2
  exit 2
fi

readonly run_id="$(date -u +%Y%m%dT%H%M%SZ)"
readonly output_dir="${1:-${project_root}/build/linux-write-goal2/${run_id}}"
mkdir -p "${output_dir}"
...
readonly benchmark_filter='^TinyLSM/WriteFlushLatency(Async|Sync)/'
...
/usr/bin/time -v ./run benchmark -- \
  ...
  --benchmark_out="${output_dir}/write-goal2.json" \
...
cmake -DINPUT="${output_dir}/write-goal2.json" \
  -P scripts/verify_write_benchmark_json.cmake \
  2>&1 | tee "${output_dir}/verification.log"

printf 'Linux Goal 2 artifacts: %s\n' "${output_dir}"
