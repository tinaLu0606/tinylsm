...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'run_linux_read_goal1.sh must run inside the fixed Linux VM\n' >&2
  exit 2
fi

readonly run_id="$(date -u +%Y%m%dT%H%M%SZ)"
readonly output_dir="${1:-${project_root}/build/linux-read-goal1/${run_id}}"
mkdir -p "${output_dir}"
...
readonly benchmark_filter='^TinyLSM/(ReadMemTableHit|ReadSingleTableHit|ReadMultiTableHit|ReadMultiTableMiss|ReadRepeatedHit|ScanSingleTableRepeated|ScanMultiTableRepeated|ConcurrentRead[1248])/'
...
./run benchmark -- \
  ...
  --benchmark_out="${output_dir}/read-goal1.json" \
...
cmake -DINPUT="${output_dir}/read-goal1.json" \
  -P scripts/verify_read_benchmark_json.cmake \
  2>&1 | tee "${output_dir}/verification.log"

printf 'Linux Goal 1 artifacts: %s\n' "${output_dir}"
