...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'run_linux_snapshot_goal4.sh must run inside the fixed Linux VM\n' >&2
  exit 2
fi

readonly run_id="$(date -u +%Y%m%dT%H%M%SZ)"
readonly output_dir="${1:-${project_root}/build/linux-snapshot-goal4/${run_id}}"
mkdir -p "${output_dir}"
...
readonly benchmark_filter='^TinyLSM/Snapshot(MaterializedScan|Iterator|RetentionHeld|RetentionReleased|WriterOverlap)'
...
/usr/bin/time -v ./run benchmark -- \
  ...
  --benchmark_out="${output_dir}/snapshot-goal4.json" \
  --benchmark_out_format=json \
  --benchmark_report_aggregates_only=true \
  --benchmark_display_aggregates_only=true \
  --benchmark_enable_random_interleaving=true \
  2>&1 | tee "${output_dir}/benchmark.log"
cmake -DINPUT="${output_dir}/snapshot-goal4.json" \
  -P scripts/verify_snapshot_benchmark_json.cmake \
  2>&1 | tee "${output_dir}/verification.log"

printf 'Linux Goal 4 artifacts: %s\n' "${output_dir}"
